<template>
  <div class="app-container">
    ...
    <div class="text-right" style="margin-bottom:15px;">
        <el-button type="primary" icon="el-icon-plus" @click="$router.push('/device/brand/editor')">新增</el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column label="类型" align="center">
        <template slot-scope="scope">
          <div v-if="scope.row.type == 1">设备</div>
          <div v-else-if="scope.row.type == 2">配件</div>
          <div v-else>其他</div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="left">
        <template slot-scope="scope">
          <el-button type="text" icon="el-icon-edit" @click="$router.push('/device/brand/editor?id=' + scope.row.id)">修改</el-button>
...
export default {
  ...
  methods: {
    async getPage(pageNo) {
      ...
      let res = await BrandApi.getPage(this.reqvo)
    ...
    removeById(id) {
        this.$confirm('确定要删除吗？', '提示', {
            ...
            BrandApi.removeById(id).then(res => {
                if (res.code == 0) {
                    this.getPage(1)
